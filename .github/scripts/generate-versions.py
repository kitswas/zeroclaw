...
(main, stable, v0.7.5, v0.8.0-beta-1, etc.) and generates a versions.json
...
def main():
    ...
    version_pattern = re.compile(
        r'^(main|stable|v\d+\.\d+\.\d+(-[a-z0-9.-]+)?)$', re.IGNORECASE
    )
    dirs = []
    for d in os.listdir('.'):
    ...
    dirs.sort(key=lambda x: (x != 'main', x != 'stable', x), reverse=False)
